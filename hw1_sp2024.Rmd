...
top_five <- baseball %>%
  ...
  # select 2010 through 2014 (inclusive)
  select(team, year, diff_log) %>%
  # compute the payroll increase
  group_by(team) %>%
  summarize(total_increase = sum(diff_log)) %>%
  # sort and select the top 5 largest team-payroll increases
  arrange(desc(total_increase)) %>%
  slice_max(total_increase, n = 5) %>%
  select(team, total_increase)
top_five
```

By computing the sum of the log increase in payroll from 2010 to 2014 (inclusive), we found the cumulative log difference in payroll increases. Using this method, we found that the five teams with the highest increase in their payroll changes were, in descending order: Los Angeles Dodgers, Washington Nationals, San Diego Padres, Texas Rangers, and San Francisco Giants.
